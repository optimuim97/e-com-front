<template>
  <a
    ...
    class="fixed bottom-6 right-6 z-50 flex items-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
    :class="expanded ? 'px-4 py-3' : 'p-3.5'"
  ...
  >
    ...
    <span
      v-if="!expanded"
      class="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-40"
    />
    ...
    <span
      class="relative text-sm font-semibold whitespace-nowrap overflow-hidden transition-all duration-300"
      :class="expanded ? 'max-w-xs opacity-100' : 'max-w-0 opacity-0'"
    >
      Support WhatsApp
    </span>
  </a>
